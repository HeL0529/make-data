...
class Model(nn.Module):
    ...
    def cal_kl_div(self, latents, mean, var):
        if self._cuda:
            return torch.mean(
                self.cal_log_prob(latents, mean, var) - self.cal_log_prob(latents, torch.zeros(mean.size()).cuda(),
                                                                          torch.ones(var.size()).cuda()))
        else:
            return torch.mean(
                self.cal_log_prob(latents, mean, var) - self.cal_log_prob(latents, torch.zeros(mean.size()),
                                                                          torch.ones(var.size())))
    ...
    def cal_target_loss(self, inputs, classifier_weights, target):
        ...
        target_loss = criterion(outputs, target.long())
